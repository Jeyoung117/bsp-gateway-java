...
public final class QueryImpl implements Query {
    ...
    @Override
    public BspTransactionOuterClass.SubmitResponse evaluatetoCorfu() {
        try {
            BspTransactionOuterClass.SubmitResponse response = channel.queryByChaincodetoCorfu(request);
            return response;
        } catch (ProposalException | InvalidArgumentException e) {
            throw new GatewayRuntimeException(e);
        }
